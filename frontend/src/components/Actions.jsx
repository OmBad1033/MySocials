...
function Actions({ post }) {
  ...
  const [userPost, setUserPosts] = useRecoilState(postsAtom);

  const handleLikeAndUnlike = async () => {
    if (!user) {
      showToast("Error", "Please login to like", "error");
    }
    if (isLiking) return;
    setIsLiking(true);
    try {
  ...
  const handleReply = async () => {
    ...
    try{
      ...
      const updatedPost = userPost.map((singlePost) => {
        if (singlePost._id === post._id) {
          return { ...singlePost, replies: [...singlePost.replies, data] };
        } else {
          return singlePost;
        }
      })
  ...
  return (
    <>
      <Flex flexDirection={"column"}>
        <Flex gap={3} my={3} mx={2} onClick={(e) => e.preventDefault()}>
          <svg
            aria-label="Like"
